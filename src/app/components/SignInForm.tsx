...
const SignInForm: React.FC<SignInFormProps> = ({ isLoading, setIsLoading, setError }) => {
  ...
  return (
    <form className="space-y-6" onSubmit={handleSignIn}>
      ...
      <button
        type="submit"
        className="w-full bg-gradient-to-r from-green-400 to-green-300 text-white py-3 rounded-full font-semibold text-lg hover:from-green-500 hover:to-green-400 focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
        disabled={isLoading}
      >
        {isLoading ? (
          <>
            <DotSpinner className="mr-2" />
