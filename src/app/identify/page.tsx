...
import CollapsibleBox from "../../app/components/ColapsibleBox";
import plantData from "../data/plants.json";

// Define type for categoryDisplayNames
interface CategoryDisplayNames {
  [key: string]: string;
}

// Define type for plant data
interface Plant {
  id: number;
  name: string;
  image: string;
...
const PlantIdentifier = () => {
  const { plants } = plantData as { plants: Plant[] };
  const params = useParams();
  const router = useRouter();
  const categorySlug = params?.categorySlug as string | undefined;

  // Find the category name that matches the current slug
  const categoryObject = categories.find((cat) => cat.slug === categorySlug);
  const selectedCategory = categoryObject ? categoryObject.name : "All plants";
  ...
  // Effect to filter plants when category or filters change
  useEffect(() => {
    let filteredPlants = selectedCategory === "All plants"
      ? plants
      : plants.filter((plant) => plant.category.toLowerCase() === selectedCategory.toLowerCase());

    // Apply trait filters if any are selected (placeholder for future implementation)
    if (Object.values(traits).some(value => value)) {
      // TODO: Implement trait filtering logic when plant data includes traits
    }

    // Apply life cycle filters if any are selected (placeholder for future implementation)
    if (Object.values(lifeCycle).some(value => value)) {
      // TODO: Implement life cycle filtering logic when plant data includes life cycle
    }

    setDisplayedPlants(filteredPlants);
  }, [selectedCategory, traits, lifeCycle, plants]);
  ...
  const getPlantCount = (categoryName: string) => {
    if (categoryName === "All plants") {
      return plants.length;
    }
    return plants.filter((plant) => plant.category.toLowerCase() === categoryName.toLowerCase()).length;
  };
  ...
  // Debugging logs
  console.log('Unique categories in plants.json:', [...new Set(plants.map(p => p.category))]);
  console.log('categorySlug:', categorySlug);
  console.log('selectedCategory:', selectedCategory);
  console.log('displayedPlants:', displayedPlants);
          ...
          <main className="w-full md:w-2/3 lg:w-3/4 md:pl-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-6">
              {displayedPlants.length > 0 ? (
                ...
                <p className="text-gray-600">No plants found for this category.</p>
              )}
            </div>
          </main>
        </div>
